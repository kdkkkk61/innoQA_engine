...
[두 가지 스캔 단계]
  test_modal_ui_patterns      : ADD 모달 스캔
    - 정책 생성 없이 바로 스캔
    - ADD 모달에서 탭 이동 차단 → tag 동작 테스트 자동 스킵
  test_modal_ui_patterns_edit : EDIT 모달 스캔
    - [AUTO]_ui_scan 임시 정책 생성 → EDIT 모달 스캔 → 정책 삭제
...
class TestRansomDetectPolicyScan:
    ...
    def test_modal_ui_patterns(self, logged_in_page, settings, request):
        ...
        검증 항목:
          ...
          - tag_input       × 4 (ADD 모달 탭 차단 → 동작 테스트 자동 스킵)
        """
