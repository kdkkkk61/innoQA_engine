...
class TestSecureZoneTemplateManageFolderScenario3Action(SecureZoneTemplateManageFolderBase):
    ...
    def test_scenario3s_path_overflow(self, logged_in_page, settings):
        ...
        for elem, setting, kw in cases:
            ...
            shots = None
            if "서버" in msg:
                # 리턴 재생 2컷(사용자 지시 2026-07-03): ①오류 위치(3000자 필드) ②그때 뜨는 경고 모달.
                # 경고는 helper 가 읽고 바로 dismiss → 입력값이 남아있으므로 '추가' 재클릭으로 재발생시켜 캡처.
                shot_field = self._shot(f"povf_{elem}_입력상태", highlight=page.page.locator(sel),
                                        caption=f"1. {elem} — 3000자 입력 상태(오류 위치)")
                shot_modal = None
                try:
                    page._content_btn("추가").evaluate("el => el.click()")
                    page.page.locator(page.SEL_CONFIRM_MODAL_OPENED).wait_for(state="attached", timeout=6000)
                    page.page.wait_for_timeout(300)
                    shot_modal = self._shot(f"povf_{elem}_서버오류모달",
