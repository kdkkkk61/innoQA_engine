"""시큐어존 템플릿(시큐어 드라이브) 시나리오 공통 base — 방식 B (list_page + sc0).

정책 _base 의 _ss(불일치 필드 crop 캡처)/_r(ScanResult 빌더) 재사용. PAGE_ID + cleanup 만 다름.
cleanup 라이프사이클(연계): sc1 시작 = delete_all_test_data([AUTO]+[AUTO_날짜]) / sc5c = delete_all_auto([AUTO]만).
...
class SecureZoneTemplateBase:
    ...
    def _add(self, status: str, label: str, detail: str = "", sc: int = 0,
        ...
        screenshot=False 면 fail/warn 이어도 캡처 생략(sc0 변경카드처럼 캡처 무의미할 때)."""
        try:
            nm = self._request.node.name
            m = re.match(r"test_scenario(\d+)([a-z])_", nm)
            if m:
