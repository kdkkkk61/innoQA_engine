...
### 자동 분류 트리거 (시나리오 2~5 영역으로 확장)

`discovered_new` 발견은 시나리오 1 카드 등록으로 끝나지 않는다. **시나리오 2~5 영역에도
자동으로 검증 카드를 등록**해 신규 기능이 각 시나리오별로 어떻게 동작하는지 보고서에
표시한다.

| 시나리오 | 자동 등록 카드 | pattern |
|---|---|---|
| 2 | 신규 요소 동작 검증 (`[신규]` 라벨) | text_input / plain_checkbox / toggle_checkbox |
| 3 | 신규 text/number 자동 채우기 | `discovered_fill` |
| 4 | EDIT 모달 로드값 default 유지 확인 | initial_state (`[신규]` 라벨) |
| 5 | ON/OFF 케이스 시점 DOM 값 캡처 | `discovered_case` |

상세 매핑·라벨 컨벤션은 `test_scenario_standard.md` "자동 분류 메커니즘 (B-1 시리즈)" 참조.
각 시나리오 문서의 "자동 검증 카드" 섹션에 동작 정의 기재.

→ 시나리오 1 책임 범위: **회귀 감지 시작점**. 이후 흐름은 각 시나리오 영역에서 처리.
