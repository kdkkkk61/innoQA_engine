...
>   ※ 방식 B(list_page) 페이지는 이 스캔을 **sc0(test_scenario0_scan.py)로 분리** — 설계·삭제/숨김 감지·
>     스키마 제외정책은 `docs/scenario_0_scan.md` 참조. (아래 표의 '시나리오 1 스캔'은 방식 A 기준 서술)
>
...
1. **[AUTO] 접두사 필수** — 생성/삭제는 `[AUTO]`로 시작하는 항목만.
   다음 테스트(sc6 연계)에 남겨둘 날짜본 항목은 **`[AUTO_<MMDD>]`**(예 `[AUTO_0629]`) 사용 — 'KEEP' 용어 폐기, 날짜 유무로만 구분. 고정 `[AUTO_KEEP]`은 다른 날 이전 본 잔존 시 이름 충돌이라 날짜본으로 대체(nPouch 레거시 코드는 `[AUTO_KEEP]` 유지). 상세: `docs/scenario_6_suite_setup.md`
...
□ 시나리오 4 구현 → 실행 확인
□ 시나리오 5 구현 → 실행 확인
□ 시나리오 6 구현 → 실행 확인 (다른 페이지가 사용할 데이터가 있으면 [AUTO_<MMDD>] 날짜본 항목 생성 후 남겨둠)
```
