...
| 검색 동작 | 리스트/picker 검색창 + 검색버튼/Enter 있는 경우 | 검색어 입력 → 버튼/Enter → 결과 필터 반영 (미반영 시 결함). 매칭 확실한 값으로 검증 |

> 위 중 해당 없는 항목은 `[SKIP] {케이스명} — 해당 없음 (이유)` 으로 기록
> sc1 은 검색창/버튼 **존재**만, sc3 은 검색 **실행→필터 반영 동작** 검증 (책임 분리).
> 검증 대상: 리스트 페이지 검색 + 각 picker(템플릿 선택 등) 내부 검색 — 같은 컴포넌트라도 모달별로 확인.
