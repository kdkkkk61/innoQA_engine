...
## 스캔 결과 기반 yaml 갱신 절차 (2026-05-06 추가)

시나리오 1의 DOM 스캔이 신규/제거 요소를 감지한 경우 yaml 갱신 절차:

### 🆕 신규 요소 발견 시
```
1. 보고서에서 "discovered_new" 항목 확인
2. Chrome MCP 또는 브라우저로 해당 페이지 직접 접속
3. 요소의 의도 파악:
   - 의도된 신규 기능 → yaml에 정식 추가 (Chrome MCP로 동작 검증 후)
   - 임시/실험 요소 → yaml note에 "스캔 무시" 표시 또는 제외 목록 추가
4. yaml 추가 시 confirmed_date / 확인 결과 명시 (기존 절차 동일)
```

### ❌ 제거된 요소 발견 시
```
1. 보고서에서 "discovered_missing" 항목 확인
2. 제품팀에 의도된 제거인지 확인:
   - 의도된 제거 → yaml에서 항목 삭제 + 변경 이력 주석에 기록
   - 회귀 (의도치 않은 제거) → 제품 버그 보고
```

### 자동 회귀 메커니즘 검증
주기적으로 yaml 일부 항목을 주석 처리 → 테스트 실행 → "🆕 신규" 카드 등장 확인.
주석 복원 후 재실행 → 카드 사라짐 확인. 메커니즘 동작 보장.
