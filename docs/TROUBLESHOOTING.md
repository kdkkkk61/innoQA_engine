...
## 병합 결함 카드 — 대표 항목 캡처만 싣고 나머지 항목 캡처는 유실 + 카운트 22↔9 모순 — 2026-07-13

- **증상(사용자 지적)**: ①타입·시나리오 병합 카드의 스크린샷이 1개 항목 것만 — 각 검출은 별도 테스트의 재현 컷인데 묶으면서 유실. ②섹션 제목 "확정된 결함 (22건)"인데 카드는 9장. 게다가 탭 클릭 시 JS 가 카드 수로 다시 세서 22→9 로 바뀌는 내부 모순도 있었음(초기값=raw 결과 합, 탭 필터=카드 수).
- **원인**: html_reporter 병합 렌더가 '결과 전수 표기'(2026-07-09)는 구현했지만 **스크린샷은 대표(첫) 항목의 extra 만 읽음**. 카운트는 초기 렌더(fail+warn+error 합)와 JS 재계산(카드 수)이 서로 다른 지표.
- **수정**(core/html_reporter.py):
  - 병합 카드 캡처 전수: 그룹 항목 각각의 screenshots 를 `{"divider": "<시나리오> — <변형 머리>"}` 구분 머리와 함께 전부 표기(.ss-div). 장수 표기는 실제 이미지 수로.
  - 카운트 병기: 제목 "결함 N건 · 검출 M개소"(N=카드, M=raw 결과). 카드에 `data-spots`+`검출 N개소` chip, 탭 필터 JS 도 두 수치를 같은 정의로 갱신 — 지표 이원화 해소(고유 이슈 수 vs 검출 지점 수).
- **검증**: scratchpad 스모크(가짜 병합 2항목+단독 1) — 카드 2·검출 3개소·구분 머리 2·3장 표기 전부 확인.
- **잔여**: 대시보드 확정 경로(app.py finalize)는 여전히 단일 스크린샷·단일 카운트 — 다중 캡처 렌더 반영 작업(보류 해제됨)에서 함께 처리.
- **교훈**: 병합은 "표기를 압축"하는 것이지 "증거를 압축"하는 게 아니다 — 결과 전수 원칙은 텍스트만이 아니라 캡처에도 적용.

상태: [RESOLVED] (실 리포트 확인은 다음 run)

---

## L2 상태 셀 3중 오판 최종 정정 — 표시는 정상, 클래스 판독이 오탐 원인 (오탐 카드 3종 제거) — 2026-07-13

- **증상(사용자 지적)**: sc3t 카드 텍스트는 "계속 ON — 표시 결함"인데 첨부 캡처의 토글은 회색(OFF·비활성 표시) — 저장값(비활성)과 오히려 일치. "비활성이면 회색이 맞지 않냐"는 지적.
- **Chrome 프로브 실측**([AUTO]_probe_status, 비활성/활성 2행 대조 + 편집 양방향):
  - `label.switch` 의 'on' 클래스 = **항상 붙음**(상태 무관, 무의미).
  - `input#status.checked` = 저장값의 **반전**(비활성=checked true, 활성=false).
  - CSS `.switch.on input:checked+.slider`(회색, 특이도 최고)가 `.switch.on .slider`(파랑)를 이김 → **비주얼 = 클래스 XOR checked** → 결과적으로 저장값을 정확히 표시.
  - 등록(비활성/활성)·편집(활성↔비활성) **전부 행 표시 즉시 갱신 — 표시 기능 정상**.
- **오판의 3층**: ①7/9 "input 미바인딩, 클래스가 진실"(468b03e) — 반전 바인딩을 미바인딩으로 오독 ②"저장값 무관 항상 ON 렌더" — 항상 붙는 클래스를 읽은 것 ③7/10 "편집 열닫 갱신 트리거"(5744cf4) — 오탐 위에 세운 해석. **sc3t/sc4g/sc4s 의 '표시 결함' 카드 전부 오탐.**
- **수정**: page `l2_status_display_on()`(슬라이더 computed 색 판독 — 비주얼 진실) 신설, sc3t/sc4g(_row_item_status_on)/sc4s 판독 교체. 결함 분기 자체는 유지(진짜 표시 결함이면 여전히 검출).
- **부수 정정(sc4r, 같은 원인 계열)**: "태그 3000자 — 경고·저장 요청 모두 없이 무반응" 문구는 수동 프로브(JS setter 미동기)의 오관찰 — run 로그 실측은 **PUT /processTag/szTag 500 발송됨**(raw 서버 오류 은폐, 프로세스 설명 오버플로와 동일 계열). sc4r 에 response 수집 추가, 문구를 실측 분기로. 측정 스코프(허용 템플릿, 거부=편집 불능으로 검증 불가) 명시.
...
## 커밋 직후 L2 판독 stale 5회째 — 근원을 커밋 함수(l3_add_message)로 이동 — 2026-07-13

- **증상(13:22 run, 14:45·62tests)**: sc4m/sc4p FAILED IndexError(등록 직후 `l2_rows()` 빈 리스트) + sc3k "등록=0건" fail 오탐(캡처에는 등록 행 1건이 선명 — 이미지 검수로 확증) + 그 오탐이 _R 재생 35s 유발.
- **원인**: 성능 수정으로 `l3_add_message` 반환이 빨라지자(closed 감지 ~0.2s), 그동안 느린 반환이 가려주던 "커밋 직후 무대기 판독" 인라인 지점 3곳이 동시에 드러남. `_ensure_item` 근원 수정(4회째)은 그 헬퍼를 쓰는 곳만 보호 — 인라인 등록 지점은 무방비.
- **수정**: 호출부가 아니라 **커밋 함수가 보장** — `l3_add_message` 의 'closed'(커밋) 경로에서 `_l2_wait_stable()`(행 수 연속 2회 일치 + 1건 이상, cap 2s) 후 반환. 모든 현재·미래 호출부가 자동 보호.
- **교훈**: 같은 함정이 헬퍼를 갈아타며 재발하면(호출부→_ensure_item→인라인) 보장 지점을 한 단계 더 아래(상태를 바꾼 함수 자신)로 내려야 끝난다. 그리고 **성능 개선은 timing 마스킹을 벗긴다** — 빨라진 직후 run 은 새 flaky 를 드러내는 검사이기도 하다.

상태: [RESOLVED] (재실행 검증 대기 — 사용자 실행)

---

## picker 행 로드 미대기 + 커밋 후 지연 알림 유실 — 13:57 run 잔여 2건 (이미지 검수로 확증) — 2026-07-13

- **증상(13:57 run, 14:41·62tests)**: ①sc3c FAILED — picker 열림 직후 행 0건 순회 → picked=[] → 미선택 '추가' 경고 연쇄, 이후 확인 버튼 native click timeout(래퍼 w=0 이중 존재). ②sc3g warn 오판 — 부분중복 커밋의 '생략' 안내가 msg='' 로 잡힘. **캡처에는 알림이 떠 있음**(카드-사진 모순, 이미지 검수 2번째 성과).
- **원인**: ①`l3_register` 가 `picker.wait_open()`(모달만 보장) 후 행 로드(비동기)를 안 기다리고 순회. ②부분중복 커밋은 **L3 닫힘 → 알림 표시** 순서 — `_l3_commit_outcome` 이 L3 detach 를 보는 즉시 'closed' 반환해 늦게 도착한 알림을 유실(구코드 500ms sleep 이 우연히 잡던 케이스).
- **수정**: ①`l3_register` 행 attach 대기 추가 + `ProcessPicker.confirm` 에 JS click fallback(래퍼 확인 버튼 JS click 정상 — Chrome 실측 2026-07-13). ②'closed' 경로에서 `_l2_wait_stable` 후 300ms 내 알림 attach 시 메시지 회수·반환.
- **교훈**: "커밋 완료"의 신호는 하나가 아니다 — L3 닫힘·알림 표시·목록 갱신이 **순서 보장 없이** 따로 온다. 완료 판정은 목적(메시지 수집 vs 상태 판독)에 필요한 신호가 다 들어올 때까지.

상태: [RESOLVED] (재실행 검증 대기 — 사용자 실행)

---

## 일괄 삭제 체크 유실 — 재렌더 경합으로 '삭제할 항목을 체크해 주세요' (stale 계열 마지막 변형) — 2026-07-13

- **증상(16:50 run)**: sc5d 최종 cleanup 이 일괄 삭제에서 에러 모달 '삭제할 항목을 체크해 주세요' 로 FAILED — 체크 evaluate 는 실행됐는데 삭제 클릭 시점엔 체크 0.
- **원인**: 검색("[AUTO") 직후 목록 재렌더가 비동기 — 체크 클릭 후 행이 교체되며 체크 유실. 오늘 계열(액션→비동기 재렌더→상태 유실)의 cleanup 판. 고정 300ms 로는 경합을 못 막음.
- **수정**: `_delete_matching` — 체크 후 `:checked` 수를 실측 확인, 미달이면 재체크(최대 4회) 후에만 삭제 클릭. '체크해 주세요' 에러 모달은 raise 대신 dismiss 후 외곽 루프 재시도(자기회복).
- **교훈**: "클릭했다"와 "반영됐다"는 다른 사실 — 일괄 조작도 반영 수를 읽어 확인 후 다음 단계로. (커밋 3신호 대기·행 안정화와 같은 원칙의 체크박스 판)

상태: [RESOLVED] (재실행 검증 대기 — 사용자 실행)

---

## 오버플로 카드 과병합 — 증상이 다른 생성(raw 오류)·수정(조용한 미저장)이 한 카드 — 2026-07-14

- **증상(사용자 지적)**: "설명 3000자 → 저장 처리" 카드(3.12·4.09 검출 7개소)에 서로 다른 두 증상이 동거 — 생성 경로 4건=서버 오류 알림·미커밋 / 수정 경로 3건=경고 없이 조용한 미저장. 읽는 사람이 "공통 증상"으로 오해.
- **원인**: merge_key `szproc_desc_ovf::PROCESS` 가 경로(3↔4) 세로 병합용으로 설계돼 **증상 구분 없이** 묶음. 가로 병합 정책("결과 동일하면 묶고 갈리면 분리", 2026-07-10)이 세로 병합엔 미적용이었음.
- **수정**: merge_key 를 증상 축으로 분리 — `::raw`(알림형)/`::silent`(무반응 미저장)/`::clip`(절단)/`::bypass`(검증 우회 저장). 제목도 증상 중심으로("→ raw 서버 오류" / "→ 조용한 미저장"). 프로세스(sc3l/sc4i)·태그(sc3v/sc4r) 4곳 적용. 같은 증상이면 경로·타입 가로/세로 병합은 종전대로 유지 — 수정 경로가 나중에 raw 오류를 내면 자동 재병합.
- **교훈**: 병합의 축은 "같은 필드"가 아니라 **"같은 증상"** — 필드가 같아도 경로별 증상이 갈리면 별개 결함이다.

상태: [RESOLVED] (재실행 검증 대기 — 사용자 실행)

---

## 병합 카드 등급 미승격 — fail 검출이 warn 대표에 가려 노랑 카드로 표시 — 2026-07-14

- **증상(사용자 지적)**: 요약은 빨강(fail) 2건인데 결함 카드에 빨강이 없음 — 거부 편집 불능 카드(검출 3개소)가 warn 항목(3000자 경위)이 대표라 노랑 BUG 로 렌더, fail 2개소(정상값 복구 불가·태그 편집 불가)가 그 안에 숨음("노랑 안에 빨강").
- **원인**: 병합 카드의 배지·심각도를 그룹 **첫 항목의 status** 로 결정 — 등급 혼재 그룹에서 최고 등급 승격 로직 부재.
- **수정**(core/html_reporter.py): ①병합 카드 등급 = 구성 항목 최고 등급(fail/error 하나라도 있으면 빨강 BUG·심각도 높음) ②결과 전수 행에 [높음] 마커 — 어느 검출이 fail 인지 카드 안에서 구분. 스모크(warn+fail 혼재 병합) 렌더 검증.
- **검수 루틴 추가**: 리포트 검수 시 **요약 fail 카운트 ↔ 빨강 카드 수 대조** — 승격 누락/카운트 불일치 감지.
- **교훈**: 병합은 표기 압축이지 등급 압축이 아니다 — 대표 항목의 속성(캡처·등급·결과)을 그룹 전체로 일반화하는 실수는 이번이 세 번째(캡처 유실→결과 전수→등급). "대표 1건" 패턴 자체를 의심할 것.

상태: [RESOLVED] (다음 run 리포트에서 실물 확인)
